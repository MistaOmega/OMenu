// dear imgui: Renderer Rendering for DirectX9
// This needs to be used along with a Platform Rendering (e.g. Win32)
...
//  2018-06-08: Misc: Extracted imgui_impl_dx9.cpp/.h away from the old combined DIRECTX9+Win32 example.
...
// Rendering data stored in io.BackendRendererUserData to allow support for multiple Dear ImGui contexts
...
void ImGui_ImplDX9_RenderDrawData(ImDrawData *draw_data) {
    ...
    // Backup the DIRECTX9 transform (DIRECTX9 documentation suggests that it is included in the StateBlock but it doesn't appear to)
    ...
    // Copy and convert all vertices into a single contiguous buffer, convert colors to DIRECTX9 default format.
    ...
    // Restore the DIRECTX9 transform
    bd->pd3dDevice->SetTransform(D3DTS_WORLD, &last_world);
    bd->pd3dDevice->SetTransform(D3DTS_VIEW, &last_view);
    bd->pd3dDevice->SetTransform(D3DTS_PROJECTION, &last_projection);

    // Restore the DIRECTX9 state
...
static bool ImGui_ImplDX9_CreateFontsTexture() {
    ...
    // Convert RGBA32 to BGRA32 (because RGBA32 is not well supported by DIRECTX9 devices)
