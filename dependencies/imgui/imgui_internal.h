...
// [SECTION] Rendering Engine specific hooks (imgui_test_engine)
...
enum ImGuiButtonFlagsPrivate_ {
    ...
    ImGuiButtonFlags_NoSetKeyOwner = 1
    ...
    ImGuiButtonFlags_NoTestKeyOwner = 1
...
enum ImGuiSelectableFlagsPrivate_ {
    ...
    ImGuiSelectableFlags_NoSetKeyOwner = 1
...
enum ImGuiInputFlags_ {
    ...
    ImGuiInputFlags_LockThisFrame = 1
    ...
    ImGuiInputFlags_LockUntilRelease = 1
    ...
    ImGuiInputFlags_RouteFocused = 1
    ...
    ImGuiInputFlags_RouteGlobalLow = 1
    ...
    ImGuiInputFlags_RouteGlobalHigh = 1
    ...
    ImGuiInputFlags_Repeat | ImGuiInputFlags_RepeatRateMask_ | ImGuiInputFlags_RouteMask_ |
...
struct ImGuiNavItemData {
    ImGuiWindow *Window;         // Hook,Move    // Best candidate window (result->ItemWindow->RootWindowForNav == request->Window)
    ImGuiID ID;             // Hook,Move    // Best candidate item ID
    ImGuiID FocusScopeId;   // Hook,Move    // Best candidate focus scope ID
    ImRect RectRel;        // Hook,Move    // Best candidate bounding box in window relative space
...
struct ImGuiContext {
    ...
    void *TestEngine;                         // Rendering engine user data
    ...
    // Navigation: Hook & Move Requests
    bool NavAnyRequest;                      // ~~ NavMoveRequest || NavInitRequest this is to perform early out in ItemAdd()
    bool NavInitRequest;                     // Hook request for appearing window to select first item
    bool NavInitRequestFromMove;
    ImGuiID NavInitResultId;                    // Hook request result (first item of the window, or one for which SetItemDefaultFocus() was called)
    ImRect NavInitResultRectRel;               // Hook request result rectangle (relative to parent window)
...
//#endif
    ...
    // - This is marked experimental because not all widgets are fully honoring the Set/Rendering idioms. We will need to move forward step by step.
    ...
    IMGUI_API bool TestKeyOwner(ImGuiKey key,
                                ImGuiID owner_id);                       // Rendering that key is either not owned, either owned by 'owner_id'
    ...
    IMGUI_API bool IsKeyPressed(ImGuiKey key, ImGuiID owner_id,
                                ImGuiInputFlags flags = 0);    // Important: when transitioning from old to new IsKeyPressed(): old API has "bool repeat = true", so would default to repeat. New API requiress explicit ImGuiInputFlags_Repeat.
    ...
    SplitterBehavior(const ImRect &bb, ImGuiID id, ImGuiAxis axis, float *size1, float *size2, float min_size1,
                     float min_size2, float hover_extend = 0.0f, float hover_visibility_delay = 0.0f, ImU32 bg_col = 0);
