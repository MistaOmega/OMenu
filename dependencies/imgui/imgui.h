...
enum ImGuiFocusedFlags_ {
    ...
    ImGuiFocusedFlags_RootWindow = 1 << 1,   // Rendering from root window (top most parent of the current hierarchy)
...
enum ImGuiHoveredFlags_ {
    ...
    1 << 1,   // IsWindowHovered() only: Rendering from root window (top most parent of the current hierarchy)
...
enum ImGuiConfigFlags_ {
    ...
    1 << 1,   // Master gamepad navigation enable flag. Rendering also needs to set ImGuiBackendFlags_HasGamepad.
...
// Rendering capabilities flags stored in io.BackendFlags. Set by imgui_impl_xxx or custom backend.
enum ImGuiBackendFlags_ {
    ImGuiBackendFlags_None = 0,
    ImGuiBackendFlags_HasGamepad = 1 << 0,   // Rendering Platform supports gamepad and currently has one connected.
    ImGuiBackendFlags_HasMouseCursors =
    1 << 1,   // Rendering Platform supports honoring GetMouseCursor() value to change the OS cursor shape.
    ImGuiBackendFlags_HasSetMousePos = 1
            << 2,   // Rendering Platform supports io.WantSetMousePos requests to reposition the OS mouse position (only used if ImGuiConfigFlags_NavEnableSetMousePos is set).
    ImGuiBackendFlags_RendererHasVtxOffset = 1
            << 3,   // Rendering Renderer supports ImDrawCmd::VtxOffset. This enables output of large meshes (64K+ vertices) while still using 16-bit indices.
};
...
struct ImGuiViewport {
    ...
    // Platform/Rendering Dependent Data
