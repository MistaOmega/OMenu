...
void ImGui::TableUpdateLayout(ImGuiTable *table) {
    ...
    const float width_removed = (table->HasScrollbarYPrev && !table->InnerWindow->ScrollbarY) ? g.Style.ScrollbarSize
    ...
    const float width_avail = ImMax(1.0f, (((table->Flags & ImGuiTableFlags_ScrollX) && table->InnerWidth == 0.0f)
                                           ? table->InnerClipRect.GetWidth() : work_rect.GetWidth()) - width_removed);
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
        ...
        column->NavLayerCurrent = (ImS8) (table->FreezeRowsCount > 0 ? ImGuiNavLayer_Menu
        ...
        // Possible solution to preserve last known content width for clipped column. Rendering 'table_reported_size' fails when enabling Y clipping and window is resized small.
...
ImGui::TableSetupColumn(const char *label, ImGuiTableColumnFlags flags, float init_width_or_weight, ImGuiID user_id) {
    ...
    if (table->IsInitializing) {
        ...
        // Hook default visibility/sort state
...
//-------------------------------------------------------------------------
// [Hook] 1: TableSettingsHandler_ReadXXXX()   Load and parse .ini file into TableSettings.
